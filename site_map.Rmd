...
sites_all <- read_csv('./NEON_sites/NEON_Field_Site_Metadata_20210226_0.csv') %>% 
  dplyr::select(domainID:colocated_site, latitude:longitude, state:country)
```
...
mySites <- c('HARV', 'DELA', 'OSBS', 'JERC', 'SCBI')
```
...
maps::map('state', region = myStates, add = T, fill=T, 
          col= c('darkorange', 'firebrick2', 'cyan4','springgreen1', 'purple'))  # Customize the fill color if you want!
...
text(sites$longitude, sites$latitude, sites$siteID,
     cex=1, font=2, 
     pos = c(4, 4, 4, 4, 1)) # 1 = below; 2 = left; 3 = above; 4 = right
```
