...
```{r, eval = FALSE}
data <- full_join(mammals, ticks) %>%
  full_join(., temp_monthly)

print(data)
```
```{r, eval = FALSE}
data <- full_join(data, temp_monthly) 
...
summary <- monthly %>% 
  ...
  summarize(n = n(),  # Sample size
            mean_var1 = mean(Var1), sd_var1 = sd(Var1), se_var1 = sd_var1/sqrt(n), # Mean, sd, se for var1
            mean_var2 = mean(Var2), sd_var2 = sd(Var2), se_var2 = sd_var2/sqrt(n)) # Mean, sd, se for var1
