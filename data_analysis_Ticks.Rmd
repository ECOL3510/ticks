...
## 2nd Graph (ticks vs. mammals)
```{r}
ggplot(data = monthly, aes(x = all_mammals, # name the variable that goes on the x-axis
                                    y = sum_adult,  # name the variable that goes on the y-axis
                                    col = siteID)) + # name the variable to differentiate colors
  geom_point(pch = 16, size = 3) +  # Graph these variables as a scatterplot
  theme_minimal() +
   scale_x_continuous(limits = c(0, 1500)) + # Set min/max x-axis values
  scale_y_continuous(limits = c(0, 30), # Set min/max y-axis values
                     breaks = seq(2500, 6500, 1000)) + # Specify axis break values
  scale_color_manual(values = c("darkorange","purple","cyan4", "firebrick2", "springgreen1")) + # Specify custom colors
  labs(x = "Number of White Mice", # Specify axis labels
       y = "Number of Adult Ticks", 
       color = "NEON Site")
```
...
```{r}
mammVticks <- lm(sum_adult ~ all_mammals, data = monthly)
```
...
ggplot(data = monthly,aes(x = siteID,
  ...
  labs(x = "NEON Site", # Specify x-axis label
       y = "Number of Adult Ticks") # Specify y-axis label
``` 

```{r}
ticksVsite <- aov(sum_adult ~ siteID, data = monthly)
```
...
```{r}
tukey <- tidy(TukeyHSD(ticksVsite))
print(tukey)
```
