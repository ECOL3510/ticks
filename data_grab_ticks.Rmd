...
```{r, eval = FALSE}
list2env((loadByProduct(dpID = 'DP1.10093.001',
                           site = c('DELA','JERC','SCBI'),
                           startdate = '2017-03',  # YYYY-MM
                           enddate = '2021-07')),  # YYYY-MM
         .GlobalEnv)
```
...
for (i in 1:length(files)){
  ...
  write_csv(files[[i]], file = paste0("./ticks_raw/", names(files[i]), ".csv", sep=""))
}
```
...
for (i in 1:length(files)){
  # Change the SCBI_mammals text below to indicate the name of the folder where you want to store your data
  write_csv(files[[i]], file = paste0("./mammals_raw/", names(files[i]), ".csv", sep=""))
}
```